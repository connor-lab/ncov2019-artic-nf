...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        dest="YML_IN",
        required=False,
        help="YAML file with types and their variants",
    )
    parser.add_argument(
        "-ov",
        "--output_vcf",
        dest="VCF_OUT",
        required=True,
        help="Output VCF file with typed variants",
    )
    parser.add_argument(
        "-ot",
        "--output_typing_csv",
        dest="TYPING_CSV_OUT",
        required=True,
        help="Output CSV file with typed variants",
    )
    parser.add_argument(
        "-os",
        "--output_summary_csv",
        dest="SUMMARY_CSV_OUT",
        required=True,
        help="Output variant summary CSV file",
    )
    parser.add_argument(
        "-af",
        "--allele_freq_thresh",
    ...
    parser.add_argument(
        ...
        dest="MIN_DEPTH",
        default=0,
        help="Only output variants where overall read depth is greater than this number (default: 0).",
    )
    ...
    infile.add_argument(
        ...
        dest="VCF_IN",
        required=False,
        help="Input VCF file from either ARTIC pipeline (Nanopolish or Medaka).",
    )
    infile.add_argument(
        "-t", "--tab", dest="TSV_IN", required=False, help="Input iVar tsv file."
    )
...
def csq_annotate_vcf_string(vcfString, RefIn, GffIn):
    ...
    p = run(
        ["bcftools", "csq", "-v", "0", "-f", RefIn, "-g", GffIn],
        stdout=PIPE,
        input=vcfString,
        encoding="ascii",
    )
...
def get_variant_summary(info):
    ...
    for variant in info:
        ...
        dna_r = re.compile(
            "(?P<refpos>[0-9]+)(?P<refnucl>[A-Z\*]+)>(?P<varnucl>[A-Z\*]+)"
        )
        ...
        if (
            ...
            if "frameshift" in variant["consequence"]:
                ...
                complete_aa_variant_string = (
                    aa_var["refaa"] + aa_var["refpos"] + aa_var["varaa"]
                )

        else:
            complete_aa_variant_string = "Syn." + aa_var["refpos"] + aa_var["varaa"]

        complete_dna_variant_string = (
            dna_var["refnucl"] + dna_var["refpos"] + dna_var["varnucl"]
        )

        variant_dict = {
            "gene": variant["gene"],
            "aa_var": complete_aa_variant_string,
            "dna_var": complete_dna_variant_string,
        }
...
def main(args=None):
    ...
    infos = extract_csq_info_from_vcf_string(
        csqAnnotatedVcfString, args.ALLELE_FREQ_THRESH, args.MIN_DEPTH
    )
